		...
		return Promise.all(
			data.map(async (timestamp: { start: number; end: number }) => {
				...
				const burnedZanoBig = blocks.reduce(
					(totalFee, block) => totalFee.plus(new Decimal(Number(block.total_fee))),
					new Decimal(0),
				);

				console.log(
					`BURNED ZANO: ${burnedZanoBig.toNumber()} for start: ${start} and end: ${end}`,
				);

				return burnedZanoBig.dividedBy(new Decimal(10).pow(12)).toNumber();
			}),
		);
	}
	...
	async getAvgBlockSize(data: InputData) {
		...
		return Promise.all(
			data.map(async (timestamp: { start: number; end: number }) => {
				...
				return allBlocksSize.dividedBy(Number(blocksCount)).toNumber();
			}),
		);
	}
	...
	async getConfirmedTxs(data: InputData) {
		...
		return Promise.all(
			data.map(async (timestamp: { start: number; end: number }) => {
				...
				return txs.filter((tx) => Number(height) - tx.keeper_block > 20).length;
			}),
		);
	}
